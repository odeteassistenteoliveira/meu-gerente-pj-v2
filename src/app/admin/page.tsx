...
const PLANO_LABELS: Record<string, string> = {
  starter: "Starter", essencial: "Essencial",
  profissional: "Profissional", pro: "Pro"
};
...
function FunnelBar({ label, value, total, color, icon: Icon }: {
  ...
  return (
    <div className="flex-1 min-w-0">
      <div className={`rounded-2xl border border-white/20 p-4 ${color} text-white relative overflow-hidden`}>
...
function LeadModal({ empresa, onClose, onSave }: {
  ...
  async function handleSave() {
    setSaving(true);
    await onSave(empresa.id, {
      plano, lead_status: status, observacoes: obs,
      proximo_contato: followUp || null,
      ...(resetMsg ? { total_mensagens: 0 } : {}),
    });
    setSaving(false);
    onClose();
  }

  const statusInfo = STATUS_LEAD.find(s => s.value === status);
        ...
        <div className="p-5 space-y-4">
          ...
          {(empresa.plano ?? "starter") === "starter" && (
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" checked={resetMsg} onChange={e => setResetMsg(e.target.checked)}
...
export default function AdminPage() {
  ...
  async function handleSaveLead(empresaId: string, data: Partial<Empresa>) {
    await fetch("/api/admin/update-empresa", {
      ...
      body: JSON.stringify({ empresa_id: empresaId, ...data }),
    });
    // Optimistic update
  ...
  const pagos = empresas.filter(e => (e.plano ?? "starter") !== "starter").length;
  ...
  function exportCSV() {
    const headers = ["Nome", "Email", "CNPJ", "Telefone", "Cidade", "Estado", "Setor",
      "Faturamento", "Plano", "Status Lead", "Msgs usadas", "Próx. contato", "Obs.", "Cadastro"];
    const rows = filtradas.map(e => [
      ...
      e.total_mensagens ?? 0, e.proximo_contato ?? "", e.observacoes ?? "", formatDate(e.created_at),
    ].map(v => `"${v}"`));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-2">Leads quentes = Starter que atingiu o limite de mensagens. Funil calculado sobre o total de cadastros.</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          ...
          <KpiCard label="Novos (30 dias)" value={stats.novosMes} color="text-purple-500" />
          <KpiCard label="MRR estimado" value={`R$ ${mrr.toLocaleString("pt-BR")}`} color="text-green-600" sub="Receita recorrente mensal" />
          <KpiCard label="Taxa de conversão" value={`${taxaConv}%`} color="text-amber-600" sub={`${pagos} de ${total} pagantes`} />
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/80">
                    {["Empresa / Email","Plano","Status Lead","Msgs","Local","Cadastro","Ações"].map(h => (
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtradas.map(e => {
                    ...
                    return (
                      <>
                        <tr key={e.id} className={`hover:bg-gray-50/60 transition-colors ${isQuente ? "bg-orange-50/30" : ""}`}>
                          {/* Empresa */}
                          ...
                          {/* Plano */}
                          <td className="px-4 py-3">
                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${planoColor}`}>
                              {PLANO_LABELS[e.plano ?? "starter"]}
                            </span>
                          </td>
                          {/* Status */}
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1">
                              ...
                              <span className={`text-[12px] font-mono font-semibold ${isQuente ? "text-orange-600" : "text-gray-600"}`}>
                                {e.total_mensagens ?? 0}/10
                              </span>
                            </div>
                          </td>
                          {/* Local */}
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              {e.whatsapp && (
                                <a href={`https://wa.me/55${e.whatsapp.replace(/\D/g, "")}?text=Ol%C3%A1%2C+vi+que+voc%C3%AA+est%C3%A1+usando+o+Meu+Gerente+PJ%21`}
                                  target="_blank" rel="noopener noreferrer" title="WhatsApp"
                                  className="p-1.5 rounded-lg bg-green-50 text-green-600 hover:bg-green-100">
                                  <Phone size={13} />
                                </a>
                              )}
                              <a href={`mailto:${e.user_email}?subject=Meu+Gerente+PJ`} title="Email"
                                className="p-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100">
                                <Mail size={13} />
                              </a>
                              <button onClick={() => setEditModal(e)} title="Editar lead"
                          ...
                          <tr key={`${e.id}-detail`} className="bg-blue-50/20">
                            <td colSpan={7} className="px-6 py-4">
                              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-[12px]">
                                <div><p className="text-[10px] font-semibold text-gray-400 uppercase mb-1">Setor</p><p className="text-gray-700">{SETOR_LABELS[e.setor ?? ""] || e.setor || "—"}</p></div>
                                <div><p className="text-[10px] font-semibold text-gray-400 uppercase mb-1">Faturamento</p><p className="text-gray-700">{FATURAMENTO_LABELS[e.faturamento ?? ""] || "—"}</p></div>
                                <div><p className="text-[10px] font-semibold text-gray-400 uppercase mb-1">Regime</p><p className="text-gray-700">{e.regime || "—"}</p></div>
                                <div><p className="text-[10px] font-semibold text-gray-400 uppercase mb-1">Funcionários</p><p className="text-gray-700">{e.num_funcionarios || "—"}</p></div>
                                ...
                                <div><p className="text-[10px] font-semibold text-gray-400 uppercase mb-1">Como conheceu</p><p className="text-gray-700">{e.como_conheceu || "—"}</p></div>
                                <div><p className="text-[10px] font-semibold text-gray-400 uppercase mb-1">Redes</p>
                                  <div className="flex gap-1.5">
                                    {e.instagram && <a href={`https://instagram.com/${e.instagram.replace("@","")}`} target="_blank" rel="noopener noreferrer" className="text-pink-500"><Instagram size={14} /></a>}
                                    {e.linkedin && <a href={e.linkedin.startsWith("http") ? e.linkedin : `https://linkedin.com/in/${e.linkedin}`} target="_blank" rel="noopener noreferrer" className="text-blue-600"><Linkedin size={14} /></a>}
                                    {e.site_url && <a href={e.site_url.startsWith("http") ? e.site_url : `https://${e.site_url}`} target="_blank" rel="noopener noreferrer" className="text-gray-500"><Globe size={14} /></a>}
                                  </div>
                                </div>
                                {e.observacoes && (
                                  <div className="md:col-span-2"><p className="text-[10px] font-semibold text-gray-400 uppercase mb-1">Observações</p>
                                    <p className="text-gray-700 bg-yellow-50 rounded-lg px-2 py-1.5 text-[11px]">{e.observacoes}</p>
                                  </div>
                                )}
        ...
        <p className="text-center text-xs text-gray-400 pb-4">
          Painel admin · Meu Gerente PJ · {filtradas.length} de {total} registros
        </p>
      </div>
    </div>
  );
